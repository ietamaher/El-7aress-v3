...
void NightCameraInterface::performFFC() {
    // Command to perform Flat Field Correction (FFC_MODE_SELECT, 0x0B, Set Automatic FFC)
    QByteArray command = buildCommand(0x0B, QByteArray::fromHex("0001"));
    sendCommand(command);
}

void NightCameraInterface::setDigitalZoom(quint8 zoomLevel) {
    // Command to set digital zoom (VIDEO_MODE, 0x0F)
    // Argument: 0x0004 for zoom or 0x0000 for real-time mode
...
void NightCameraInterface::setVideoModeLUT(quint16 mode) {
    // Command to set video mode (e.g., White-Hot or Black-Hot)
    // VIDEO_LUT (0x10): Argument 0x0000 (White-Hot), 0x0001 (Black-Hot)
...
void NightCameraInterface::getCameraStatus() {
    // Command to request camera status (STATUS_REQUEST, 0x06)
    QByteArray command = buildCommand(0x06, QByteArray());
    sendCommand(command);
}

// Utility to build command packets
QByteArray NightCameraInterface::buildCommand(quint8 function, const QByteArray &data) {
    QByteArray packet;
...
// CRC Calculation
quint16 NightCameraInterface::calculateCRC(const QByteArray &data, int length) {
    quint16 crc = 0x0000;
...
void NightCameraInterface::handleResponse(const QByteArray &response) {
    if (response.isEmpty()) {
...
bool NightCameraInterface::verifyCRC(const QByteArray &packet) {
...
void NightCameraInterface::handleVideoModeResponse(const QByteArray &data) {
    if (data.size() < 2) {
...
void NightCameraInterface::handleVideoLUTResponse(const QByteArray &data) {
    if (data.size() < 2) {
...
void NightCameraInterface::handleFFCResponse(const QByteArray &data) {
    qDebug() << "Flat Field Correction Response received.";
    emit responseReceived(data);
}

void NightCameraInterface::handleStatusResponse(const QByteArray &data) {
    if (data.isEmpty()) {
...
void NightCameraInterface::handleStatusError(quint8 statusByte) {
