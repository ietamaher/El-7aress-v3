...
PLCRemoteStationSensorInterface::PLCRemoteStationSensorInterface(const QString &device,
                                                 int baudRate,
                                                 int slaveId,
                                                 QObject *parent)
    : QObject(parent),
    ...
    m_reconnectAttempts(0)
{
    ...
    // Connect signals
    connect(m_modbusDevice, &QModbusClient::stateChanged, this, &PLCRemoteStationSensorInterface::onStateChanged);
    connect(m_modbusDevice, &QModbusClient::errorOccurred, this, &PLCRemoteStationSensorInterface::onErrorOccurred);

    connect(m_readTimer, &QTimer::timeout, this, &PLCRemoteStationSensorInterface::readData);
    m_readTimer->setInterval(100); // Reduced read frequency to 500 ms

    connect(m_timeoutTimer, &QTimer::timeout, this, &PLCRemoteStationSensorInterface::handleTimeout);
    m_timeoutTimer->setSingleShot(true);

}

PLCRemoteStationSensorInterface::~PLCRemoteStationSensorInterface() {
    disconnectDevice();
}

bool PLCRemoteStationSensorInterface::connectDevice() {
...
void PLCRemoteStationSensorInterface::disconnectDevice() {
    if (m_modbusDevice->state() != QModbusDevice::UnconnectedState) {
        m_modbusDevice->disconnectDevice();
    }
    m_readTimer->stop();
...
void PLCRemoteStationSensorInterface::onStateChanged(QModbusDevice::State state) {
...
void PLCRemoteStationSensorInterface::onErrorOccurred(QModbusDevice::Error error) {
...
void PLCRemoteStationSensorInterface::readData() {
    ...
    // Read Digital Inputs
    {
        ...
        if (auto *reply = m_modbusDevice->sendReadRequest(readUnit, m_slaveId)) {
            if (!reply->isFinished()) {
                connect(reply, &QModbusReply::finished, this, &PLCRemoteStationSensorInterface::onDigitalInputsReadReady);
     ...
     // Read Analog Inputs
    {
        ...
        if (auto *reply = m_modbusDevice->sendReadRequest(readUnit, m_slaveId)) {
            if (!reply->isFinished()) {
                connect(reply, &QModbusReply::finished, this, &PLCRemoteStationSensorInterface::onAnalogInputsReadReady);
                if (!m_timeoutTimer->isActive())
                    m_timeoutTimer->start(1000);
            } else {
                reply->deleteLater();
...
void PLCRemoteStationSensorInterface::onDigitalInputsReadReady() {
...
void PLCRemoteStationSensorInterface::onAnalogInputsReadReady() {
    auto *reply = qobject_cast<QModbusReply *>(sender());
    if (!reply)
        return;
...
void PLCRemoteStationSensorInterface::writeData() {
    if (!m_modbusDevice || m_modbusDevice->state() != QModbusDevice::ConnectedState)
        return;
    ...
    if (auto *reply = m_modbusDevice->sendWriteRequest(writeUnit, m_slaveId)) {
        if (!reply->isFinished()) {
            connect(reply, &QModbusReply::finished, this, &PLCRemoteStationSensorInterface::onWriteReady);
        } else {
            reply->deleteLater();
        }
    } else {
...
void PLCRemoteStationSensorInterface::onWriteReady() {
    auto *reply = qobject_cast<QModbusReply *>(sender());
    if (!reply)
        return;
...
void PLCRemoteStationSensorInterface::handleTimeout() {
    ...
    if (m_modbusDevice) {
        m_modbusDevice->disconnectDevice();
        QTimer::singleShot(delay, this, &PLCRemoteStationSensorInterface::connectDevice);
    }
}


void PLCRemoteStationSensorInterface::logError(const QString &message) {
    emit logMessage(message);
    qDebug() << "PLCRemoteStationSensorInterface:" << message;
}

// Getters and setters for digital inputs
QVector<bool> PLCRemoteStationSensorInterface::digitalInputs() const {
...
QVector<uint16_t> PLCRemoteStationSensorInterface::analogInputs() const {
    QMutexLocker locker(const_cast<QMutex*>(&m_mutex)); // Use const_cast here
    return m_analogInputs;
}

// Set digital outputs
void PLCRemoteStationSensorInterface::setDigitalOutputs(const QVector<bool> &outputs) {
    {
        QMutexLocker locker(&m_mutex);
